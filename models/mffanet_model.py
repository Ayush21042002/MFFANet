...
from models.mffanet import MFFANet

class MFFANetModel(BaseModel):

    def modify_commandline_options(parser, is_train):
        parser.add_argument('--scale_factor', type=int, default=8, help='upscale factor for MFFANet')
    ...
    def __init__(self, opt):
        ...
        self.netG = MFFANet(res_depth=opt.res_depth, norm_type=opt.Gnorm, att_name=opt.att_name, bottleneck_size=opt.bottleneck_size) 
    ...
    def backward_G(self):
        # Pix loss
        self.loss_Pix = self.criterionL2(self.img_SR, self.img_HR) * self.opt.lambda_pix
