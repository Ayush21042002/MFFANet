...
class CelebADataset(BaseDataset):
    def __init__(self, opt):
        ...
        self.imgrar = rarfile.RarFile(self.img_dir)
    ...
    def get_img_names(self,):
        ...
        img_names = filter(lambda x : x.filename != "img_align_celeba/", self.imgrar.infolist()) 
    ...
    def __getitem__(self, idx):
        # self.imgrar = rarfile.RarFile(self.img_dir)
        img_path = os.path.join(self.img_dir, self.img_names[idx].filename)
        img_file = self.imgrar.open(self.img_names[idx])
        hr_img = Image.open(img_file).convert('RGB')
        ...
        haze_hr_img = haze_hr_img*tr + A*(1 - tr)
        haze_hr_img = Image.fromarray(np.uint8(haze_hr_img)).convert('RGB')

        # downsample and upsample to get the LR image
        lr_img = haze_hr_img.resize((self.lr_size, self.lr_size), Image.BICUBIC) 
